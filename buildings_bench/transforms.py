...
class BoxCoxTransform:
    ...
    def train(self, data: np.ndarray) -> None:
        ...
        if data.shape[0] > self.max_datapoints:
            ...
            data = data[
                np.random.choice(data.shape[0], self.max_datapoints, replace=False)
            ]
    ...
    def transform(self, sample: np.ndarray) -> np.ndarray:
        ...
        return self.boxcox.transform(1e-6 + sample.flatten().reshape(-1, 1)).reshape(
            init_shape
        )
    ...
    def undo_transform(
        ...
        init_shape = sample.shape
        sample = self.boxcox.inverse_transform(sample.flatten().reshape(-1, 1)).reshape(
            init_shape
        )
...
class LatLonTransform:
    ...
    def __init__(self):
        ...
        for idx, cr in enumerate(census_regions):
            cr_df = pd.read_csv(
                metadata_path / f"map_of_pumas_in_census_region_{idx+1}_{cr}.csv",
                header=0,
            )
            # keys are cr_df['GISJOIN'], values are (cr_df['latitude'], cr_df['longitude'])
            self.puma_to_centroid.update(
                dict(zip(cr_df["GISJOIN"], zip(cr_df["latitude"], cr_df["longitude"])))
            )
        ...
        for k, v in self.puma_to_centroid.items():
            ...
            v = (
                ...
                - np.array([self.lat_means, self.lon_means])
            ) / np.array([self.lat_stds, self.lon_stds])
    ...
    def transform_latlon(self, latlon: np.ndarray) -> np.ndarray:
        ...
        """
        return (latlon - np.array([self.lat_means, self.lon_means])) / np.array(
            [self.lat_stds, self.lon_stds]
        )
...
class TimestampTransform:
    ...
    def undo_transform(self, time_features: np.ndarray) -> np.ndarray:
        ...
        return (
            np.stack([day_of_year, day_of_week, hour_of_day], axis=1)
            .astype(np.int32)
            .reshape(init_shape)
        )
