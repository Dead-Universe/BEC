...
class MetricsManager:
    ...
    def _compute_scoring_rule(
        ...
        if y_distribution_params is None:
            raise ValueError(
                "y_distribution_params must be provided to compute scoring rule."
            )
    ...
    def __call__(
        ...
        **kwargs
        ...
        if building_types_mask is None:
            building_types_mask = (
                building_type == BuildingTypes.COMMERCIAL_INT
...
class DatasetMetricsManager:
    ...
    def get_building_from_dataset(
        self, dataset_name: str, building_id: str
    ...
    def add_building_to_dataset_if_missing(
        self, dataset_name: str, building_id: str
    ) -> None:
        ...
        if building_id not in self._metrics[dataset_name]:
            ...
            self._metrics[dataset_name][building_id] = MetricsManager(
                deepcopy(self.metrics_list), deepcopy(self.scoring_rule)
            )
        ...
        if dataset_name is None:  # summarize all datasets
            for dataset_name in self._metrics.keys():
                ...
                for building_id in self._metrics[dataset_name].keys():
                    summary[dataset_name][building_id] = self._metrics[dataset_name][
                        building_id
                    ].summary()
        else:
            summary[dataset_name] = {}
        ...
        for dataset_name in summary.keys():
            ...
            for building_id in summary[dataset_name].keys():
                ...
                for building_type in summary[dataset_name][building_id].keys():
                    # for each metric
                    for metric_name in summary[dataset_name][building_id][
                        building_type
                    ].keys():
                        ...
                        if (
                            summary[dataset_name][building_id][building_type][
                                metric_name
                            ].type
                            == MetricType.SCALAR
                        ):
                        ...
                        # if the metric is a list of scalars
                            ...
                            == MetricType.HOUR_OF_DAY
                        ):
                            multi_hour_value = summary[dataset_name][building_id][
                                building_type
                            ][metric_name].value
                            for hour in range(multi_hour_value.shape[0]):
                                rows.append(
                                    [
                                        ...
                                        building_id,
                                        building_type,
                                        metric_name.split("-")[0] + "_" + str(hour),
                                        MetricType.HOUR_OF_DAY,
                                        multi_hour_value[hour].item(),
                                    ]
                                )
                ...
                for building_id in summary[dataset_name].keys():
                    for building_type in summary[dataset_name][building_id].keys():
                        if (
                            ...
                            in summary[dataset_name][building_id][building_type]
                        ):
                            score = summary[dataset_name][building_id][building_type][
                                self.scoring_rule.name
                            ].value
                            for hour in range(score.shape[0]):
                                rows.append(
                                    [
                                        ...
                                        building_id,
                                        building_type,
                                        self.scoring_rule.name + "_" + str(hour),
                                        score[hour].item(),
                                    ]
                                )
    ...
    def __call__(
        ...
        **kwargs
        ...
        self._metrics[dataset_name][building_id](
            y_true, y_pred, building_types_mask, **kwargs
        )
