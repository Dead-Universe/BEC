...
class DeepAutoregressiveRNN(BaseModel):
    ...
    def __init__(
        ...
        super(DeepAutoregressiveRNN, self).__init__(
            context_len, pred_len, continuous_loads
        )
        ...
        if not self.ignore_exogenous:
            ...
            self.encoder = nn.LSTM(
                256, hidden_size, num_layers=lstm_layers, batch_first=True
            )
            ...
            self.encoder = nn.LSTM(
                1, hidden_size, num_layers=lstm_layers, batch_first=True
            )
            self.decoder = nn.LSTM(
                1, hidden_size, num_layers=lstm_layers, batch_first=True
            )
            ...
            if k < self.pred_len:
                ...
                if self.for_pretraining:
                    ...
                    next_decoder_input = torch.cat(
                        [
                            next_decoder_input[:, : -embedded_pred.shape[-1]],
                            embedded_pred,
                        ],
                        dim=1,
                    )
                    ...
                    next_decoder_input = torch.cat(
                        [next_decoder_input[:, :-1], outputs], dim=1
                    )
