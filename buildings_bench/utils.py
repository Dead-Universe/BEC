...
def time_features_to_datetime(time_features: np.ndarray, year: int) -> np.array:
    """
    ...
    return np.array(
        [
            datetime.datetime(year, 1, 1, 0, 0, 0)  # January 1st
            + datetime.timedelta(
                days=int(doy - 1), hours=int(hod), minutes=0, seconds=0
            )
...
def get_puma_county_lookup_table(metadata_dir: Path) -> pd.DataFrame:
    ...
    df_has_weather = lookup_df[
        (lookup_df.weather_file_2012 != "No weather file")
