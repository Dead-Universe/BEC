...
try:
    ...
    print(
        "Faiss is not installed. Follow instructions at "
...
class LoadQuantizer:
    ...
    def train(self, sample: np.ndarray) -> None:
        ...
        if self.with_merge:
            ...
            self.original_order_to_sorted_centroid_indices = np.zeros_like(
                sorted_indices
            )
            ...
            for i in range(1, len(index_to_value_map)):
                ...
                if (
                    index_to_value_map[i] - previous_centroid_value
            ...
            self.original_order_to_merged_centroid_map = np.zeros(
                len(index_to_value_map), dtype=np.int32
            )
    ...
    def save(self, output_path: Path) -> None:
        ...
        np.save(
            output_path / "kmeans_centroids_K={}.npy".format(self.K),
            self.kmeans.centroids,
        )
        if self.with_merge:
            np.save(
                output_path
                / "kmeans_original_to_sorted_indices_K={}.npy".format(self.K),
                self.original_order_to_sorted_centroid_indices,
            )
            np.save(
                output_path / "kmeans_original_to_merged_map_K={}.npy".format(self.K),
                self.original_order_to_merged_centroid_map,
            )
            np.save(
                output_path / "kmeans_merged_centers_K={}.npy".format(self.K),
                self.merged_centroids,
            )
    ...
    def load(self, saved_path: Path) -> None:
        ...
        if self.with_merge:
            ...
            self.original_order_to_merged_centroid_map = np.load(
                saved_path / "kmeans_original_to_merged_map_K={}.npy".format(self.K),
                allow_pickle=True,
            )
            self.merged_centroids = np.load(
                saved_path / "kmeans_merged_centers_K={}.npy".format(self.K),
                allow_pickle=True,
            )
        ...
        if "cuda" in self.device:
            ...
            self.kmeans.index = faiss.index_cpu_to_gpu(
                faiss_res, local_rank, self.kmeans.index
            )
            self.kmeans.centroids = (
                torch.from_numpy(self.kmeans.centroids).float().to(self.device)
            )
            self.kmeans.centroids.squeeze()
            if self.with_merge:
                self.original_order_to_sorted_centroid_indices = (
                    torch.from_numpy(self.original_order_to_sorted_centroid_indices)
                    .long()
                    .to(self.device)
                )
                self.original_order_to_merged_centroid_map = (
                    torch.from_numpy(self.original_order_to_merged_centroid_map)
                    .long()
                    .to(self.device)
                )
                self.merged_centroids = (
                    torch.from_numpy(self.merged_centroids).float().to(self.device)
                )
                self.merged_centroids.squeeze()
            # print(f'Kmeans quantizer moved to GPU: {type(self.kmeans.index)}')

    def transform(
        self, sample: Union[np.ndarray, torch.Tensor]
