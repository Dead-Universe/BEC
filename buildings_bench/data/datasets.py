...
class TorchBuildingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        load_features = (
            self.df["power"]
            .iloc[seq_ptr - self.context_len : seq_ptr + self.pred_len]
            .values.astype(np.float32)
        )
        if self.apply_scaler_transform != "":
            load_features = self.load_transform.transform(load_features)
        time_features = self.time_transform.transform(
            self.df.index[seq_ptr - self.context_len : seq_ptr + self.pred_len]
        )
        latlon_features = (
            self.normalized_latlon.reshape(1, 2)
            .repeat(self.context_len + self.pred_len, axis=0)
            .astype(np.float32)
        )
        if self.building_type == BuildingTypes.RESIDENTIAL:
            building_features = BuildingTypes.RESIDENTIAL_INT * np.ones(
                (self.context_len + self.pred_len, 1), dtype=np.int32
            )
        elif self.building_type == BuildingTypes.COMMERCIAL:
            building_features = BuildingTypes.COMMERCIAL_INT * np.ones(
                (self.context_len + self.pred_len, 1), dtype=np.int32
            )
        ...
        if self.weather_df is None:
            ...
            for col in weather_df.columns:
                ...
                sample.update(
                    {
                        col: weather_transform.transform(weather_df[col].to_numpy())[0][
                            ..., None
                        ]
                    }
                )
...
class TorchBuildingDatasetFromParquet:
    ...
    def __init__(
        ...
        for parquet_data, building_latlon, building_type in zip(
            parquet_datasets, building_latlons, building_types
        ):
            ...
            if weather_inputs:  # load weather data
                ...
                assert (
                    datetime.datetime.strptime(
                        weather_df["date_time"].iloc[0], "%Y-%m-%d %H:%M:%S"
                    ).strftime("%m-%d")
                    == "01-01"
            ...
            for building_dataframe in dfs:
                ...
                building_dataframe.rename(
                    columns={building_dataframe.columns[0]: "power"}, inplace=True
                )
                self.building_datasets[b_file] = TorchBuildingDataset(
                    building_dataframe,
                    building_latlon,
                    building_type,
                    context_len,
    ...
    def __init__(
        ...
        *,  # 👈 新增关键字参数
        ...
        for building_year_file in building_year_files:
            ...
            df = pd.read_csv(
                data_path / (building_year_file + ".csv"),
                index_col=0,
                header=0,
                parse_dates=True,
            )
            ...
            if weather_inputs and (ds_name == "SMART" or ds_name == "BDG-2"):
                weather_df = pd.read_csv(
                    data_path / f"{ds_name}" / f"weather_{name}.csv",
                    index_col=0,
                    header=0,
                    parse_dates=True,
                )
...
class PandasBuildingDatasetsFromCSV:
    ...
    def __init__(
        ...
        weather_df = None
        if weather_inputs:
            assert weather_inputs == [
                "temperature"
            ], "Only temperature is supported by PandasBuildingDatasetsFromCSV for now."

            self.weather_transform_path = (
                data_path / "metadata" / "transforms" / "weather"
            )
            ds_name = building_year_files[0].split("/")[0]
            if ds_name != "SMART" and ds_name != "BDG-2":
        ...
        for building_year_file in building_year_files:
            ...
            for bldg_name, df in zip(bldg_names, bldg_dfs):
                if self.features == "engineered":
                    self._prepare_data_with_engineered_features(
                        bldg_name, df, year, weather_df
                    )
    ...
    def _prepare_data_with_engineered_features(self, bldg_name, df, year, weather_df):
        ...
        df = pd.get_dummies(
            df,
            columns=["day_of_week", "hour_of_day", "month_of_year", "weekend"],
            dtype=np.int32,
        )
...
class PandasTransformerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        time_features = (
            self.df[["day_of_year", "day_of_week", "hour_of_day"]]
            .iloc[seq_ptr - self.context_len : seq_ptr + self.pred_len]
            .values.astype(np.float32)
        )
        sample = {
