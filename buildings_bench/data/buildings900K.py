...
class Buildings900K(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.weather_inputs:  # build a puma-county lookup table
            ...
            for col in self.weather_inputs:
                ...
                self.weather_transforms[-1].load(
                    self.metadata_path / "transforms" / "weather" / col
                )
    ...
    def __getitem__(self, idx):
        ...
        df = pq.read_table(
            str(
                self.dataset_path
                / self.building_type_and_year[int(ts_idx[0])]
                / "timeseries_individual_buildings"
                / self.census_regions[int(ts_idx[1])]
                / "upgrade=0"
                / f"puma={ts_idx[2]}"
            ),
        ...
        time_features = self.time_transform.transform(
            df["timestamp"].iloc[seq_ptr - self.context_len : seq_ptr + self.pred_len]
        )
        ...
        latlon_features = self.spatial_transform.transform(ts_idx[2]).repeat(
            self.context_len + self.pred_len, axis=0
        )

        # residential = 0 and commercial = 1
        building_features = np.ones(
            (self.context_len + self.pred_len, 1), dtype=np.int32
        ) * int(int(ts_idx[0]) % 2 == 0)
