...
def train(df_tr, df_val, args, model, transform, loss, lr, device):
    torch_train_set = PandasTransformerDataset(
        df_tr, sliding_window=24, weather=args.weather
    )
    ...
    torch_val_set = PandasTransformerDataset(
        df_val, sliding_window=24, weather=args.weather
    )
    ...
    if params is None:
        raise ValueError(
            "No parameters provided for fine-tuning. Did you mean to run with --eval_zero_shot?"
        )
    ...
    for epoch in range(args.max_epochs):
        ...
        if patience_counter > args.patience:
            print(
                f"early stopping after epoch {epoch} with patience={args.patience}, best val loss {best_val}"
            )
            break
...
def transfer_learning(args, model_args, results_path: Path):
    ...
    if not args.dont_subsample_buildings:
        ...
        if len(args.hyper_opt) > 0:
            with open(
                metadata_dir / "transfer_learning_hyperparameter_tuning.txt", "r"
            ) as f:
                target_buildings += f.read().splitlines()
        else:
            with open(
                metadata_dir / "transfer_learning_commercial_buildings.txt", "r"
            ) as f:
                target_buildings += f.read().splitlines()
            with open(
                metadata_dir / "transfer_learning_residential_buildings.txt", "r"
            ) as f:
                target_buildings += f.read().splitlines()
    ...
    for dataset in args.benchmark:
        ...
        for count, (building_name, bldg_df) in enumerate(dataset_generator, start=1):
            ...
            print(
                f"dataset {dataset} building {building_name} {count}/{num_of_buildings}"
            )
            ...
            training_end_timestamp = training_start_timestamp + pd.Timedelta(
                days=args.num_training_days - 30
            )

            train_date_range = pd.date_range(
                start=training_start_timestamp, end=training_end_timestamp, freq="H"
            )
            ...
            test_end_timestamp = test_start_timestamp + pd.Timedelta(
                days=180
            )  # test on subsequent <= 180 days
            test_set = test_set[test_set.index <= test_end_timestamp]

            print(
                f"fine-tune set date range: {training_set_.index[0]} {training_set_.index[-1]}, "
            ...
            torch_test_set = PandasTransformerDataset(
                test_set, sliding_window=24, weather=args.weather
            )
            ...
            with torch.no_grad():
                for batch in test_dataloader:
                    ...
                    if args.device == "cuda":
                        with torch.cuda.amp.autocast():
                            predictions, distribution_params = tuned_model.predict(
                                batch
                            )
                    else:
                    ...
                    if args.apply_scaler_transform != "":
                        ...
                        if (
                            not args.ignore_scoring_rules
                            and args.apply_scaler_transform == "standard"
                        ):
                            ...
                            not args.ignore_scoring_rules
                            and args.apply_scaler_transform == "boxcox"
                        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not compute a scoring rule",
    )
    parser.add_argument(
        "--variant_name",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="", help="Path to a checkpoint to load."
    )
