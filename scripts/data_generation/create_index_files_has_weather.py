...
def main(args):
    ...
    time_series_dir = Path(
        os.environ.get("BUILDINGS_BENCH", ""),
        "Buildings-900K",
    ...
    with open(
        Path(os.environ.get("BUILDINGS_BENCH", "")) / "metadata" / "withheld_pumas.tsv",
        "r",
    ) as f:
        # tab separated file
        line = f.readlines()[0]
    ...
    for building_type_and_year, by in enumerate(building_years):
        ...
        if "amy2018" in by:
            train_hours = int(
                (
                    train_amy2018_timerange[1] - train_amy2018_timerange[0]
                ).total_seconds()
                / 3600
            )
            ...
            val_hours = int(
                (val_timerange[1] - val_timerange[0]).total_seconds() / 3600
            )
            print(f"AMY2018 Val hours: {val_hours}")

        else:
            train_hours = int(
                (train_tmy_timerange[1] - train_tmy_timerange[0]).total_seconds() / 3600
            )
            ...
            for pum_file in tqdm(pum_files):
                ...
                if not os.path.basename(pum_file) in withheld_pumas:
                    for bldg_id in bldg_ids:
                        ...
                        s_start = np.random.randint(
                            1, 24
                        )  # skip the 00:00:00 timestamp in the beginning of the file
                        for s_idx in range(
                            s_start,
                            train_hours - (args.context_len + args.pred_len),
                            args.sliding_window_stride,
                        ):
                            seq_ptr = str(args.context_len + s_idx).zfill(
                                4
                            )  # largest seq ptr is < 10000
                        ...
                        if "amy2018" in by:
                            ...
                            for s_idx in range(
                                ...
                                - (args.context_len + args.pred_len),
                                args.sliding_window_stride,
                            ):
                                seq_ptr = str(args.context_len + s_idx).zfill(
                                    4
