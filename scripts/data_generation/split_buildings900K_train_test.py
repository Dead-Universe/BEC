...
def main():
    ...
    for building_type_and_year, by in enumerate(building_years):
        ...
        for census_region_idx, pum in enumerate(pumas):
            ...
            pum_files = [
                os.path.basename(x) for x in glob.glob(str(pum_path / "puma=*"))
            ]
            for withheld_puma in withheld_pumas:
                if withheld_puma in pum_files:
                    ...
                    shutil.copytree(
                        pum_path / withheld_puma, benchmark_by_pum_path / withheld_puma
                    )
                    shutil.rmtree(pum_path / withheld_puma)
                    print(
                        f"Moved {pum_path / withheld_puma} to {benchmark_by_pum_path / withheld_puma}"
                    )
