...
def download_bdg2_data(savedir):
    ...
    print(
        f"use git lfs to download the data from the git repo {url} "
...
def download_sceaux_data(savedir):
    ...
    print(
        f"Log into Kaggle then download the dataset from {url}. Unzip the file archive.zip and copy "
...
def download_smart_data(savedir):
    url_Homes = [
        "https://lass.cs.umass.edu/smarttraces/2017/HomeA-electrical.tar.gz",
...
def download_borealis_data(savedir):
    ...
    print(
        f"Navigate to {url} and download the .zip datasest. Unzip the file and copy "
...
if __name__ == "__main__":
    ...
    print(
        "WARNING: This script will try to automatically download and process the datasets in BuildingsBench."
    ...
    df = pd.read_csv(
        dataset_dir / "Electricity" / "LD2011_2014.txt", sep=";", index_col=0
    )
    ...
    for col in df.columns:
        ...
        if df[col].dtype == "object" and col != "index":
            df[col] = df[col].apply(
                lambda x: float(x.replace(",", ".") if type(x) is str else x)
            )
    ...
    for year in years:
        ...
        bldgs = bldgs.interpolate(
            method="linear", axis=0, limit=24 * 7, limit_direction="both"
        )
        ...
        # Save to csv
        bldgs.to_csv(
            dataset_dir / "Electricity" / f"LD2011_2014_clean={year}.csv",
            index=True,
            header=True,
        )
    ...
    try:
        ...
        for s in sites:
            ...
            for year in years:
                ...
                sit = s.replace("_", "")
                bldgs.to_csv(
                    dataset_dir / "BDG-2" / f"{sit}_clean={year}.csv",
                    header=True,
                    index=True,
                )
    except:
    ...
    try:
        ...
        house = pd.read_csv(
            dataset_dir / "Sceaux" / "household_power_consumption.txt",
            sep=";",
            header=0,
        )
        ...
        for year in years:
            ...
            # Save to csv
            h.to_csv(
                dataset_dir / "Sceaux" / f"Sceaux_clean={year}.csv",
                header=True,
                index=True,
            )
    except:
    ...
    for idx in range(len(homes)):
        ...
        for year in years:
            try:
                ...
                house_ = pd.read_csv(
                    dataset_dir / "SMART" / f"{home}/{year}/{home}-{meter}_{year}.csv"
                )
            except:
                continue
            ...
            if missing_frac <= 0.1 and zeros_frac <= 0.1:
                # Linearly interpolate missing values
                house_ = house_.interpolate(
                    method="linear", axis=0, limit=24 * 7, limit_direction="both"
                )
                # Fill remaining missing values with zeros
                house_ = house_.fillna(0)
                house_.to_csv(
                    dataset_dir / "SMART" / f"{home}_clean={year}.csv",
                    header=True,
                    index=True,
                )
    ...
    try:
        homes = glob.glob(
            str(dataset_dir / "Borealis" / "6secs_load_measurement_dataset" / "*.csv")
        )
        ...
        for home in homes:
            house_ = pd.read_csv(
                dataset_dir
                / "Borealis"
                / "6secs_load_measurement_dataset"
                / f"{home}.csv"
            )
            ...
            for year in years:
                ...
                if missing_frac <= 0.1:
                    ...
                    # Save to csv
                    h.to_csv(
                        dataset_dir / "Borealis" / f"{home}_clean={year}.csv",
                        header=True,
                        index=True,
                    )
    ...
    if skip == "y":
        ...
        homes_gz = glob.glob(
            str(
                ...
                / "IDEAL"
                / "sensordata"
                / "home*_electric-mains_electric-combined.csv.gz"
            )
        )
        # gunzip each file
        for h in homes_gz:
        ...
        homes = glob.glob(
            str(
                ...
                / "IDEAL"
                / "sensordata"
                / "home*_electric-mains_electric-combined.csv"
            )
        )
        ...
        for h in homes:
            ...
            for year in years:
                ...
                if missing_frac <= 0.1:
                    ...
                    # save
                    ho.to_csv(
                        dataset_dir / "IDEAL" / f"{homeid}_clean={year}.csv",
                        header=True,
                        index=True,
                    )
            del home
    ...
    all_LCL = glob.glob(
        str(dataset_dir / "LCL" / "Small LCL Data" / "LCL-June2015v2*.csv")
    )
