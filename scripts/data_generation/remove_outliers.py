...
def distance_filter(series, window_size):
    ...
    # select threshold
    average_gap = (
        series.rolling(window=24, center=True).max().mean()
        - series.rolling(window=24, center=True).min().mean()
    )
...
if __name__ == "__main__":
    ...
    """
    benchmarks = ["BDG-2", "Borealis", "Electricity", "IDEAL", "LCL", "Sceaux", "SMART"]
