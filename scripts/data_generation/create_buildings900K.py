...
def main(args):
    ...
    eulp_dir = os.path.join(
        args.eulp_dir, "end-use-load-profiles-for-us-building-stock", "2021"
    )
        ...
        for year in years:
            ...
            for res_or_com in ["resstock", "comstock"]:
                ...
                for puma_id in list_of_pumas:
                    print(
                        f"processing {puma_id} to store buildings as a parquet file..."
                    )

                    target_puma_path = os.path.join(
                        target_path, os.path.basename(puma_id)
                    )
                    ...
                    df = spark.read.parquet(puma_id).select(
                        [
                    ...
                    df = df.withColumn(
                        "timestamp", df.timestamp - F.expr("INTERVAL 15 minutes")
                    )
                    df = df.withColumn(
                        "timestamp",
                        F.date_trunc("hour", df["timestamp"]).cast(TimestampNTZType()),
                    )

                    df = df.groupBy("timestamp", "bldg_id").agg(
                        F.sum("`out.site_energy.total.energy_consumption`").alias(
                    ...
                    df = (
                        ...
                        .pivot("bldg_id")
                        .agg(F.first("total_energy_consumption"))
                    )
                    # fill na with 0
                    df = df.fillna(0)
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--eulp_dir", type=str, required=True, help="Path to raw EULP data."
    )
    args.add_argument(
        "--output_dir",
        type=str,
