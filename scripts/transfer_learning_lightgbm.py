        ...
        for building_name, bldg_df in dataset_generator:
            ...
            forecaster = ForecasterAutoreg(
                regressor=LGBMRegressor(
                    max_depth=-1, n_estimators=100, n_jobs=24, verbose=-1
                ),
                lags=lag,
            )
            forecaster.fit(
                ...
                exog=training_set[
                    [key for key in training_set.keys() if key != "power"]
                ],
            )
            ...
            for i in range(pred_days):
                ...
                predictions = forecaster.predict(
                    ...
                    exog=ground_truth[
                        [key for key in test_set.keys() if key != "power"]
                    ],
                )
                ...
                metrics_manager(
                    dataset_name,
                    f"{building_name}",
                    torch.from_numpy(ground_truth["power"].values)
                    .float()
                    .view(1, 24, 1),
