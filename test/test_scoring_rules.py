...
class TestRPS(unittest.TestCase):
    ...
    def test_bin_widths(self):
        rps = RankedProbabilityScore()
        bin_values = torch.FloatTensor([1.0, 5.0, 7.0])
        y_true = torch.FloatTensor([[[0.0], [1.0]]])  # batch_size 1, seq_len 2, 1
        y_pred_logits = torch.FloatTensor(
...
class TestContinuousRPS(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.obs = torch.from_numpy(
            np.random.normal(loc=self.mu, scale=self.sig, size=shape)
        )
    ...
    def test_continuous_rps_correct(self):
        ...
        n = 1000
        q = np.linspace(0.0 + 0.5 / n, 1.0 - 0.5 / n, n)
        ...
        expected = crps_ensemble(self.obs.squeeze(2).numpy(), forecasts, axis=0)
