...
class TestMetrics(unittest.TestCase):
    ...
    def test_mae(self):
        mae = evaluation.metrics_factory("mae", types=[evaluation.MetricType.SCALAR])
        ...
        self.assertEqual(mae.value, torch.FloatTensor([0]))
        self.assertEqual(mae.UNUSED_FLAG, False)

    def test_cvrmse(self):
        cvrmse = evaluation.metrics_factory(
            "cvrmse", types=[evaluation.MetricType.SCALAR]
        )
        ...
        cvrmse.mean()
        self.assertEqual(cvrmse.value, torch.FloatTensor([0]))
        self.assertEqual(cvrmse.UNUSED_FLAG, False)

    def test_mbe(self):
        mbe = evaluation.metrics_factory("mbe", types=[evaluation.MetricType.SCALAR])
        ...
        mbe.mean()
        self.assertEqual(mbe.value, torch.FloatTensor([0]))
