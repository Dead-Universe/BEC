...
class TestMetricsManager(unittest.TestCase):
    ...
    def test_create_dataset_metrics_manager_single_metric(self):
        metrics_manager = MetricsManager(metrics=metrics_factory("mae"))
        ...
        self.assertEqual(
            metrics_manager.metrics[BuildingTypes.RESIDENTIAL][0].name, "mae"
        )
    ...
    def test_create_dataset_metrics_manager_multiple_metrics(self):
        ...
        self.assertEqual(
            metrics_manager.metrics[BuildingTypes.RESIDENTIAL][1].name, "rmse"
        )

    def test_create_dataset_metrics_manager_with_scoring_rule(self):
        metrics_manager = MetricsManager(scoring_rule=scoring_rule_factory("rps"))

        self.assertEqual(len(metrics_manager.scoring_rules), 2)
        self.assertEqual(
            metrics_manager.scoring_rules[BuildingTypes.RESIDENTIAL].name, "rps"
        )
        self.assertEqual(
            metrics_manager.scoring_rules[BuildingTypes.COMMERCIAL].name, "rps"
        )

    def test_create_dataset_metrics_manager_with_scoring_rule_and_metrics(self):
        metrics_manager = MetricsManager(
            metrics=metrics_factory("mae"), scoring_rule=scoring_rule_factory("rps")
        )
    ...
    def test_reset(self):
        ...
        self.assertEqual(
            metrics_manager.metrics[BuildingTypes.RESIDENTIAL][0].UNUSED_FLAG, True
        )
...
class TestBenchmarkMetricsManager(unittest.TestCase):
    ...
    def test_add_building_to_dataset_if_missing(self):
        ...
        metrics_manager.add_building_to_dataset_if_missing(
            dataset_name="test", building_id="0001"
        )
    ...
    def test_call_benchmark_metrics_manager(self):
        ...
        self.assertEqual(
            buliding_mm.metrics[BuildingTypes.RESIDENTIAL][0].name, "cvrmse"
        )
